...
python q1.py --queue discovered --data node2_images.csv > discovered_queue.log &
sleep 1
python q1.py --queue tilled > tilling_queue.log &
sleep 1
python tilling.py --name tilling09 --scale_bands 299 --output_folder $NODE_LFS_PATH --queue_in discovered.queue.url --queue_out tilled.queue.url > tilling1.log &
python tilling.py --name tilling10 --scale_bands 299 --output_folder $NODE_LFS_PATH --queue_in discovered.queue.url --queue_out tilled.queue.url > tilling2.log &
python tilling.py --name tilling11 --scale_bands 299 --output_folder $NODE_LFS_PATH --queue_in discovered.queue.url --queue_out tilled.queue.url > tilling3.log &
